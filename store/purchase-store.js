...
class PurchaseStore {
  ...
  static async create(dataDir, kaikei, { fresh = false } = {}) {
    ...
    // ユーザーはマスタ（init 時に投入・冪等）。保存サイクルの対象外（read-only マスタ）
    ...
    if (fresh) {
      await store.client.query('TRUNCATE purchases, payables, journal_entries, journal_lines, pending_links, counters, meta, change_history');
    }
    await store.#hydrate();
  ...
  constructor(dataDir, kaikei) {
    ...
    this.data = { purchases: [], payables: [], entries: [], pendingLinks: [], counters: { purchase: 0, order: 0, payable: 0, entry: 0 }, users: [], changeHistory: [] };
  }
  ...
  async #hydrate() {
    ...
    const data = { purchases, payables, entries, pendingLinks, counters, users, changeHistory };
  ...
  async #saveNow() {
    ...
    try {
      ...
      const putPurchase = `INSERT INTO purchases
        (id, item, qty, unit_price, amount, requester, department, note, status, order_no, received_total, journal, history, correction, created_at)
        ...
        ON CONFLICT (id) DO UPDATE SET
          ...
          journal=EXCLUDED.journal, history=EXCLUDED.history, correction=EXCLUDED.correction, created_at=EXCLUDED.created_at`;
      for (const p of d.purchases) {
        await c.query(putPurchase, [p.id, p.item, p.qty, p.unitPrice, p.amount, p.requester, p.department,
          ...
          p.correction ? JSON.stringify(p.correction) : null, p.createdAt]);
      }
  ...
  get(id) {
    ...
    // v6 ステップ 3: 変更履歴を添付する（詳細画面・仕訳詳細から参照）
    return { ...purchase, changeHistory: this.data.changeHistory.filter((h) => h.targetId === id) };
  }
    ...
    return this.data.entries
      ...
      .map((e) => {
        ...
        return {
          ...
          vendor: null, // ステップ 8（仕入先マスタ）で埋める
